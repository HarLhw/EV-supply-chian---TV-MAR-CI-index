#20260509lhw：先尝试这个代码，如果不报错就可行。可能因为R的版本或者库的版本，以及具体数据的问题，报错。
#数据文件是 factordata_week.mat
#请直接检查 代码合理性，
################################################################################
## 
...
ksiLMAR  # 
#53:104 18.05847   [1] 667190.1 #这是我们 股价指数53:104的值
#############################################################################
# t1 <- Sys.time() 
...
result_list <- list()
t1 <- Sys.time()
result_list <- foreach(i=1, .packages = c("pracma","rTensor", "tensor","MASS"))%do%{
  end_row <- i + 51
  result <- list()
  set.seed(i)
  data_use <- rowstd(data_raw_use[i:end_row,,])
  #data_use <- rowstd(data_raw_use[i:min(i+51, nrow(data_raw_use)),,])
...
################################@@@@@@@@@@@@
result_list <- foreach(i=1:469, .packages = c("pracma","rTensor", "tensor","MASS"))%dopar%{
  
  result <- list()
  set.seed(i)
  data_use <- rowstd(data_raw_use[i:c(i+51),,])
  #data_use <- rowstd(data_raw_use[i:min(i+51, nrow(data_raw_use)),,])
  est_mle_tv <- LMARest_cept(data_use,Cvalue_est_orig,LMARinterval)
  result$A_hat_1 <- est_mle_tv$A_hat_1
  result$SIGMA_hat_1 <- est_mle_tv$SIGMA_hat_1
...
result_list <- foreach(
  i = 1:469, 
  .packages = c("rTensor", "MASS"),  # 必须加载的依赖包
  .errorhandling = "remove"         # 自动跳过报错，不中断
) %dopar% {
...
cat("成功计算结果数量：", length(result_list), "\n")
###################################！！！！！！！！
