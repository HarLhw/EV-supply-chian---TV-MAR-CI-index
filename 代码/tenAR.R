...
tenAR.sim <- function(t, dim, R, P, rho, cov, A=NULL, Sig=NULL){
  ...
  if (cov == "mle"){
    if (missing(Sig) || is.null(Sig)){
      ...
      Sig.true = Sig
      Sig.true.sqrt <- lapply(1:K, function(i){
        pracma::sqrtm(Sig.true[[i]])$B  #Square root
      })
    }
  }
  ...
  for (i in c((P+1):(500 + t))){ # burning number = 500, 500+t 改为P+t
    if (cov == "iid"){
...
Myown_tenAR.sim <- function(t, dim, R, P, A, Sig, Initialvalue){
  
  K <- length(A[[1]][[1]])
  x <- array(0, c(t+P,prod(dim)))
  ...
  for (i in c((P+1):(P + t))){ # burning number = 500, 500+t 改为P+t
  ...
  return(array(x[c((P+1):(P+t)),], c(t, dim)))
}
...
tenAR.MLE <- function(xx, R, P, init.A=NULL, init.sig=NULL, niter=150, tol=1e-5, print.true = FALSE){
  ...
  return(list(A=A.new, SIGMA=Sig.new, niter=iiter, Sig=Sig, res=res, BIC=bic, data=xx))
}
