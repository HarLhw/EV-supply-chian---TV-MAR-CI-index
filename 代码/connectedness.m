...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% connectedness index as in Diebold and Yilmaz(2009,2012,2014)
...
for ss = 1 : ny
    for j= 1 : ny
        ...
        if Pesaran_Shin  == 1 
            theta(ss,j) = Sigma(j,j)^(-1/2) * theta(ss,j); % Equation (1) in DY(2012)            
        end
    end
end
...
C.Index         = sum(sum(Theta0))*100 / ny ; % overall index of connectedness
% C.FromAllToUnit = sum(Theta0,2)*100 / (ny-1) ;    % connectedness from all to unit i 
% C.FromUnitToAll = sum(Theta0,1)'*100 / (ny-1) ;   % connectedness from unit i to all 
% get rid of ny-1
C.FromAllToUnit = sum(Theta0,2)*100  ;    % connectedness from all to unit i 
C.FromUnitToAll = sum(Theta0,1)*100 ;   % connectedness from unit i to all 
C.Net           = C.FromUnitToAll - C.FromAllToUnit';   % connectedness from unit i to all 
C.theta         = theta; 
C.Theta         = Theta*100;  %\tilde{\theta}_{ij}^{g}(H) \time 100, Equation (2) in DY(2012)
